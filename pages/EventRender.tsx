      ...
      case 5:
        return (
          <div className="space-y-5">
            ...
            <div>
              <label htmlFor="render-model" className="block text-xs font-semibold text-stone-600 dark:text-stone-400 uppercase tracking-wider mb-1.5">Calidad del render</label>
              <select id="render-model" value={renderModel} onChange={(e) => setRenderModel(e.target.value as RenderModel)} disabled={isLoading}
                className="w-full p-2.5 rounded-lg bg-white dark:bg-stone-950 border border-stone-300 dark:border-stone-700 text-stone-900 dark:text-stone-100 text-sm focus:ring-2 focus:ring-stone-400 outline-none transition disabled:opacity-40">
                {RENDER_MODEL_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
              {renderModel === 'pro' && (
