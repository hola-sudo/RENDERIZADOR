...
// Utility function to resize images before upload
...
const App: React.FC = () => {
  ...
  const handleSelectApiKey = useCallback(async () => {
    if (window.aistudio && typeof window.aistudio.openSelectKey === 'function') {
      ...
      setError("AI Studio API no disponible para selección de clave. Asegúrate de que API_KEY esté configurado en tu entorno.");
    }
  }, []);

  const handleSketchupFileChange = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files.length > 0) {
      const originalFile = event.target.files[0] as File;
      
      setCurrentGenerationProgress('Optimizando imagen...');
      setIsLoading(true); // Indicate loading while resizing
      try {
        // Resize the image to a max of 1920px on the longest side for API calls, with 0.8 quality
        const resizedFile = await resizeImageFile(originalFile, 1920, 1920, 0.8);

        setUploadedSketchupScene(resizedFile);
        setSketchupScenePreview(URL.createObjectURL(resizedFile));
        ...
        setError(`Error al optimizar la imagen: ${err.message || 'Error desconocido'}.`);
  ...
  const handleDetectSceneElements = useCallback(async () => { // Renamed handler
    ...
    setCurrentGenerationProgress('Identificando contexto y elementos principales de la escena...');

    try {
      // Use the already resized uploadedSketchupScene
      const detected = await detectSceneElements([uploadedSketchupScene], setCurrentGenerationProgress); // Pass single image in array and onProgress
      setSceneDescription(detected);
      setCurrentGenerationProgress('Elementos esenciales de la escena identificados.');
    } catch (err: any) {
      ...
      if (err.message && err.message.includes("Requested entity was not found.")) {
        ...
        setError(`Fallo al detectar elementos esenciales de la escena: ${err.message || 'Error desconocido'}.`);
      }
      setCurrentGenerationProgress('');
    } finally {
      setIsDetectingScene(false);
  ...
  const handleGenerateRender = useCallback(async () => { // Renamed handler
    ...
    setError(null);
    setCurrentGenerationProgress('Iniciando generación de render de alta resolución...');

    try {
      const result = await generateSingleRender( // Call single render function
        uploadedSketchupScene, // Use the resized file for generation as well
        sceneDescription,
        referenceImages,
        lightingType,
      ...
      if (result.error) {
        ...
        setCurrentGenerationProgress('Generación de render de alta resolución completada.');
      }
    } catch (err: any) {
  ...
  const handleStartNewScene = useCallback(() => {
    ...
    setReferenceImages([]);
    setReferenceImagePreviews([]);
    if (referenceFileInputRef.current) referenceFileInputRef.value = ''; // Clear file input
    ...
    <div className="container mx-auto p-4 md:p-8 bg-gradient-to-br from-indigo-800 to-purple-800 rounded-xl shadow-2xl max-w-4xl border border-indigo-700">
      ...
      <section className="space-y-6 mb-8 p-6 bg-indigo-800/30 rounded-lg border border-indigo-700">
        ...
        <p className="text-indigo-200">
          Sube una captura de pantalla de SketchUp para tu evento. La imagen será optimizada para la detección.
        </p>
        ...
        <button
          ...
          className="w-full py-3 px-6 rounded-full bg-indigo-600 text-white text-lg font-bold shadow-md hover:bg-indigo-700 transition duration-300 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={isLoading}
        >
        ...
        {uploadedSketchupScene && (
          <div className="mt-8 pt-6 border-t border-indigo-700">
            ...
            <button
              ...
              disabled={isDetectingScene || !hasApiKey || !uploadedSketchupScene || isLoading}
            ...
            >
              {isDetectingScene ? currentGenerationProgress.startsWith('Identificando') ? currentGenerationProgress : 'Detectando elementos...' : 'Detectar Elementos Esenciales'}
            </button>
            {isDetectingScene && !isLoading && <LoadingSpinner />}

            {sceneDescription && (
              <div className="mt-4">
                <label htmlFor="scene-description" className="block text-lg font-semibold text-indigo-200 mb-2">
                  Descripción de la Escena (Edita si es necesario para CORREGIR materiales/colores o CLARIFICAR el contexto)
                </label>
                <textarea
                  ...
                  placeholder={`Ejemplo de salida y cómo editarlo:
- Contexto del Evento: Boda elegante y minimalista al atardecer.
- Objetos Principales y Atributos:
  - Mesas redondas (forma cilíndrica) con superficie de mármol blanco pulido y base de metal dorado mate.
  - Sillas modelo Wishbone (forma curva) de madera de haya clara con asiento de cuerda trenzada beige.
  - Arreglo floral central bajo (forma esférica) con rosas blancas, peonías rosadas y follaje de eucalipto gris-verde.
- Elementos Naturales/Entorno:
  - Suelo: Césped corto y bien cuidado, color verde oliva.
  - Fondo: Pared de estuco blanco roto con una textura ligeramente rugosa.
  - Cielo: Tonos anaranjados y púrpuras de atardecer, ligeramente nublado.
- Zonas Vacías/Restringidas:
  - Primer plano: área de césped abierta y limpia.
  - Entre las mesas: espacio de tránsito despejado.
- Iluminación Base Observada: Luz suave y difusa desde la parte superior izquierda, con sombras tenues y alargadas.`}
                ></textarea>
              </div>
            )}
          </div>
      ...
      {sceneDescription.trim() !== '' && (
        <section className="space-y-6 mb-8 p-6 bg-indigo-800/30 rounded-lg border border-indigo-700">
          ...
          <input
            ...
            className="hidden"
            disabled={referenceImages.length >= MAX_REFERENCE_IMAGES || isLoading}
          />
          <button
        ...
        <section className="space-y-6 mb-8 p-6 bg-indigo-800/30 rounded-lg border border-indigo-700">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center mb-6">
            ...
            <button
              ...
              className="py-2 px-5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-full transition duration-300 ease-in-out shadow-md disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isLoading}
            >
              Cargar Archivo .json
            </button>
          ...
          <div>
            ...
            <select
              ...
              className="w-full p-3 rounded-lg bg-indigo-900 border border-indigo-700 text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition duration-200 mb-4 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={isLoading}
            >
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full p-3 rounded-lg bg-indigo-900 border border-indigo-700 text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent outline-none transition duration-200 resize-y disabled:opacity-50 disabled:cursor-not-allowed"
